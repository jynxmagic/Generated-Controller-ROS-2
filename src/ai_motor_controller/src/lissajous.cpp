...
void OffboardControl::rvolmeaControl()
{
    ...
    Eigen::Vector<float, 24> coefs {-0.315995,3.59996,-0.393737,2.3025,0.0471963,1.55924,-7.8674,0.623645,0.309091,4.14766,0.108078,5.37875,1.08379,-0.131335,6.69693,3.62777,2.25677,1.11582,3.50565,0.839767,7.43786,5.99279,5.15347,0.0340794};
...
void OffboardControl::precomputeTrajectory(float dt)
{
    ...
    // Spatial scales
    const float Rx = 0.5f;   // similar footprint as current max_radius
    const float Ry = 0.4f;
    const float Az = 0.1f;   // vertical amplitude

    // Base altitude
    const float z0 = t_z_;

    // Axis “frequencies” (in rad/s)
    // Over 40 s, these give nontrivial ratios and rich excitation.
    const float T_x = 20.0f;          // period for x
    const float T_y = 16.0f;          // period for y
    const float T_z = 30.0f;          // period for z

    const float wx = 2.0f * static_cast<float>(M_PI) / T_x;
    const float wy = 2.0f * static_cast<float>(M_PI) / T_y;
    const float wz = 2.0f * static_cast<float>(M_PI) / T_z;

    // Phase shifts to break symmetry
    const float phi_y = static_cast<float>(M_PI) / 3.0f;
    const float phi_z = static_cast<float>(M_PI) / 5.0f;
    ...
    for (int i = 0; i < N; ++i) {
        ...
        // Lissajous position
        float x = Rx * std::sin(wx * t);
        float y = Ry * std::sin(wy * t + phi_y);
        float z = z0 + Az * std::sin(wz * t + phi_z);

        // Simple yaw: align approximately with XY motion.
        // We compute a finite-difference velocity for heading.
        float yaw = 0.0f;
        if (i > 0) {
            const Eigen::Vector4f& p_prev = traj_.back();
            float vx = (x - p_prev.x()) / dt_;
            float vy = (y - p_prev.y()) / dt_;

            if (std::fabs(vx) > 1e-4f || std::fabs(vy) > 1e-4f) {
                yaw = std::atan2(vy, vx);
            } else {
                // if nearly stationary, keep previous yaw
                yaw = yaw_prev;
            }
