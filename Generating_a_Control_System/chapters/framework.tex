...
LLMs can produce functional code for most tasks, but pairing with a critic or verification step improves reliability \cite{le2024indict}.
When implemented in this manner, the LLM is proposed to refine its output based on performance, which is synonymous with self-reflection and In-Context-Learning (ICL) \cite{dong2024survey}.
By incorporating a critic model, we improve performance, similar to recent LLM approaches that use an agent to verify actions \cite{le2024indict}.
...
\end{equation}
with $\omega_{\text{max}}$ and $\omega_{\text{min}}$ representing the maximum and minimum inertia, $T$ representing the total number of iterations, and $t$ representing the current timestep.
...
For the observation state of the policy, Brownian motion noise is added to the linear and angular velocities, simulating both a random walk and a per-timestep disturbance. The noise added to the environment is somewhat complicated to write down mathematically, so we have provided a reference for those wishing to view \cite{PX4_gazebo_imu_plugin_2023}.
...
All results are provided and calculated using the Mean Squared Error (MSE) of positional reference error across all axis, in $m^2$ units.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccc}
  \hline
    \textbf{ID} & \textbf{XY Gain} & \textbf{Z Gain} \\
    \hline
    % Position
    Q\_pos & 20.418293 & 19.739102 \\
    Q\_vel &  1.104827 & 0.982716 \\
    Q\_ang & 5.291844 & 0.918472 \\
    Q\_rate & 0.108472 & 0.094821 \\
    R\_thrust & N/A & 0.091847 \\
    R\_torque & 1.048271 & 0.962819 \\
\hline
  \end{tabular}
\end{table}

We formulate the state matrix $Q$ as
\begin{equation}
    Q = \mathrm{diag}\!\big(
        Q_{\text{pos},xy,z},\,
        Q_{\text{vel},xy,z},\,
...
The state and control gains $Q$ and $R$ are obtained by running the controller in the training environment and estimating via PSO. We perform this tuning the same across all controllers. The gains for the LQR controller are provided in Table \ref{tab:lqr_gains}
...
\caption{\textbf{Results:} Comparison of SITL and real-world tracking errors for the Lemniscate and Lissajous trajectories. RVOLMEA denotes the generated controller. Subfigures (a)–(l) show axis-wise tracking error over time for the best lap (lowest MSE).}
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|c|c|c|}
    \hline
    \textbf{Controller} & \textbf{Rise time} & \textbf{Settling time} & \textbf{$s$ error} \\ \hline
    Generated controller & 1.296s & 3.832s & 0.003m \\ \hline
    PID+DOB & \textbf{0.776}s & \textbf{2.504}s& \textbf{0.001}m \\ \hline
    LQR & 1.120s & 2.792s & 0.003m \\ \hline
    \end{tabular}
\end{table}
    
Additional SITL tests were performed to confirm the UAV control system's response against more classical metrics, summarised in Table \ref{tab:classic_metrics}. Note here that $s$ error refers to the steady state error of the UAV.

On the lemniscate trajectory, the generated control system achieved a mean performance increase of 27.27\% over the best-performing hand-made controller (PID+DOB). The worst lap of the generated control system outperformed the best lap of the PID+DOB control system. The reduced variance and standard deviation within laps of the PID+DOB control system are likely due to the DOB controller reaching its disturbance-estimation limits in the presence of model inaccuracies. For the generated control system, no disturbance observer or online model estimation is available, which contributes to increased lap-to-lap variation.

SITL results are displayed visually in Figure \ref{fig:lemniscate_fullpage}. In Figure \ref{fig:lemniscate_fullpage}, we can see that the generated controller offered substantial improvement across most axis within all tasks. In some areas, specifically $P_z$ error within the Lissajous task, we can see large inital error within the control system. Postulations for this are that the sliding window used by the control system did not correctly handle a large inital input disturabance by the Lissajous trajectory.
...
Figure \ref{fig:deployment_figure} describes our deployment setup in a visual format, whereby each component is shown with relative outputs and inputs.
